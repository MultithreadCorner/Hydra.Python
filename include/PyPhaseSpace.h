...
#define PHASESPACE_GENERATE_BODY(N, RNG, BACKEND) "Generate"#BACKEND, [](hydra::PhaseSpace<N, RNG>& p, hydra::Vector4R& vec, hydra::Events<N, hydra::BACKEND::sys_t>& e) {\
        p.Generate(vec, e.begin(), e.end());\
    }

#define PHASESPACE_GENERATE_BODY_2(N, RNG, BACKEND) "Generate"#BACKEND, [](hydra::PhaseSpace<N, RNG>& p, py::sequence e1, py::sequence e2) {\
        p.Generate(e1.begin(), e1.end(), e2.begin());\
    }

#define PHASESPACE_AVERAGEON_BODY(N, RNG, BACKEND) "AverageON"#BACKEND, [](hydra::PhaseSpace<N, RNG>& p, hydra::Vector4R& v, py::function& fun, size_t nentries) {\
...
#define PHASESPACE_AVERAGEON_BODY_2(N, RNG, BACKEND) "AverageOn"#BACKEND, [](hydra::PhaseSpace<N, RNG>& p, hydra::Events<N, hydra::BACKEND::sys_t>& e, py::function& fun) {\
        return 42.0;\
    }

    /*

     #define PHASESPACE_GENERATE_BODY_2(N, RNG, BACKEND) "Generate"#BACKEND, [](hydra::PhaseSpace<N, RNG>& p, hydra::Events<N, hydra::BACKEND::sys_t>& e1, hydra::Events<N, hydra::BACKEND::sys_t>& e2) {\
        p.Generate(e1.DaughtersBegin(0), e1.DaughtersEnd(0), e2.begin());\
    }

    .def("Generate", [](hydra::PhaseSpace<N, RNG>& p, hydra::Vector4R& vec, hydra::Events<N, hydra::BACKEND::sys_t>& e) {\
        p.Generate(vec, e.begin(), e.end());\
    })\
    .def("Generate", [](hydra::PhaseSpace<N, RNG>& p, hydra::Events<N, hydra::BACKEND::sys_t>& e1, hydra::Events<N, hydra::BACKEND::sys_t>& e2) {\
        p.Generate(e1.DaughtersBegin(0), e1.DaughtersEnd(0), e2.begin());\
    })\
         .def("AverageOn"#BACKEND, [](hydra::PhaseSpace<N, RNG>& p, hydra::Vector4R& v, py::function& fun, size_t nentries) {\
        auto functor = [=](hydra::Vector4R* data) {\
            std::vector<hydra::Vector4R> vec;\
            for (int i = 0; i < 3; ++i)\
                vec.push_back(data[i]);\
    ...
    .def("AverageOn"#BACKEND, [](hydra::PhaseSpace<N, RNG>& p, hydra::Events<N, hydra::BACKEND::sys_t>& e, py::function& fun) {\
        return 42.0;\
    })
     */

#define PHASESPACE_CLASS_BODY(N, RNG, BACKEND) py::class_ < hydra::PhaseSpace<N, RNG> >(m, "PhaseSpace_"#N) \
    .def(py::init<hydra::GReal_t, const std::array<hydra::GReal_t, N>& >()) \
